...
void main() {
  test('pending side effects use a locked public API and unlocked internal path', () {
    ...
    expect(appState, contains('Future<void> _retryPendingSideEffectsUnlocked()'));
    expect(
      appState,
  ...
  test('crash logging uses synchronous rotation and writes', () {
    final source =
        File('lib/src/services/crash_reporter.dart').readAsStringSync();

    expect(source, contains('void _rotateIfNeededSync(File file)'));
    expect(source, contains('writeAsStringSync('));
    expect(source, isNot(contains('FlutterError.onError = (FlutterErrorDetails details) async')));
  });

  test('worker caches and deduplicates Google OAuth tokens', () {
    final source =
        File('workers/gemini-proxy/src/index.ts').readAsStringSync();
