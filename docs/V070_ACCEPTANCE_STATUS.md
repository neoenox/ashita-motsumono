...
補足: 画像は「同一バイナリ」のSHA-256ではなくOCRテキスト指紋による重複判定（同一内容の別画像も重複扱い）。PDFはバイナリSHA-256。共有経路（`receive_share_handler`）はセッション内TTL1時間の重複判定を備える。ギャラリー選択（`OcrPickService`）経由は永続フィンガープリントの重複判定が無い（判定は共有経路ベース。Issue #136 の [x] 判定に一致）。
