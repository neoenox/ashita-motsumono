...
証跡は既定でDocuments配下へ出力される。worktreeへ証跡ファイルを生成しない。
...
アプリを手動で開き、通知権限を許可する。初期インストールはTodo作成前に完了させる。
...
`Permission=GRANTED`でなければ先へ進まない。

## 5. 通常通知

1. Emulator画面で新規Todoを作る。
2. タイトルを`NORMAL_HHMM`とする。
3. 当日通知のみ有効にし、現在から10～15分後へ設定する。
...
`Wait`は接続、boot ID、タイムゾーン、通知権限、前面Activity、通知タイトル、heartbeat間隔を監視する。通知検出時または予定時刻+20分で証跡を保存する。

通知領域の自動展開に失敗した場合は、手動で通知領域を開いて次を実行する。
...
通知権限、未来Alarm、通知タイトル、通知領域画面、notification dump、非前面状態、Git gateが揃わない場合、PASSは拒否される。UI階層からタイトルを取得できず、人がスクリーンショットを確認した場合だけ`-ManualScreenshotVerified`を付ける。

## 6. Emulator再起動後

通常通知がPASSしてから進む。

1. 別Todo`REBOOT_HHMM`を現在から20～25分後へ設定する。
2. 保存後Homeへ移動する。
...
再起動前後のboot ID、要求時刻、`sys.boot_completed=1`確認時刻、Alarm、Notification、Activity、logcatを保存し、アプリを起動しない。boot IDが変化しなければ`BLOCKED`とする。
...
APK SHA-256、`install -r`結果、更新前後の`lastUpdateTime`、Alarm、Notification、Activity、logcatを保存する。

### broadcastを一意に確認できた場合

ログなどから`MY_PACKAGE_REPLACED`受信を一意に確認でき、通知証跡もすべて揃った場合だけPASSとする。
...
同一APKのためbroadcast受信を一意に識別できなくても、install成功、未来Alarm、通知到着、タイトル、画面、非前面状態がすべて揃っている場合だけ、限定的なINCONCLUSIVEを記録する。
...
通知未到着や証跡不足を`-InstallBroadcastUnverified`だけでClose候補にすることはできない。
...
各ケースの`Finalize`は`case-result.json`と`issue-comment.md`を生成する。3ケース完了後に集約する。
...
`ELIGIBLE_FOR_CLOSE_REVIEW`になる最低条件：

- NormalがPASS
- RebootがPASS
- install-rが`InstallBroadcastVerified`付きPASS、または通知証跡完備・`InstallBroadcastUnverified`付きINCONCLUSIVE
- 3ケースのSource SHAが同一
- そのSource SHAが集約時点の最新`origin/master`と一致

最終Close前にIssue本文との整合を再確認する。
