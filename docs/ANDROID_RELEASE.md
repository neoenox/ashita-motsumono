...
`upload-keystore.jks` をBase64化して `KEYSTORE_BASE64` に登録します。本番リリースビルドでは、署名情報に加えてAdMob本番IDとAI解析用プロキシURLも必須です。
...
Release Androidジョブは、artifactをアップロードする前に署名検証を自動実行します。検証コマンドが失敗した場合、該当artifactはアップロードされません。

APKではAndroid SDK Build Toolsの`apksigner`を使用します。

```bash
apksigner verify --verbose --print-certs app-release.apk
```

APK artifactには次も含まれます。

- `apk-signature-verification.txt`
- `APK_SHA256SUMS`

AABではJDKの`jarsigner`を使用します。

```bash
jarsigner -verify -verbose -certs app-release.aab
```

AAB artifactには次も含まれます。

- `aab-signature-verification.txt`
- `AAB_SHA256SUMS`

artifactをダウンロードした後、提出対象ファイルのSHA-256が同梱ファイルと一致することを確認します。必要に応じて上記コマンドをローカルでも再実行してください。
