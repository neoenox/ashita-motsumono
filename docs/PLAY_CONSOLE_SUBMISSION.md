...
- [ ] 通常OCRは端末上で実行し、画像を外部送信しないことを説明
- [ ] AI画像解析は同意後のみ、画像、形式、基準日、タイムゾーンをCloudflare Workers経由でGemini APIへ送信すると申告
- [ ] Workersは中継用途で、独自同期・保管バックエンドではないと説明
- [ ] Google Mobile Adsによる広告関連データ処理を申告
- [ ] Google Play Billingによる購入・復元を申告
- [ ] カメラ、画像選択、通知権限の用途を説明
- [ ] 全データ削除対象を確認
- [ ] JSONエクスポートの包含・除外データを確認
