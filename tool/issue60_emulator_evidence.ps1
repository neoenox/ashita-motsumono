[CmdletBinding()]
param(
  [Parameter(Mandatory)][ValidateSet('Preflight','BeginCase','Wait','Capture','Install','Reboot','Finalize','Aggregate')][string]$Action,
  [Parameter(Mandatory)][ValidatePattern('^[A-Za-z0-9_-]+$')][string]$CaseName,
  [ValidateSet('setup','normal','reboot','install-r')][string]$CaseType='setup',
  [ValidatePattern('^[\x20-\x7E]*$')][string]$TodoTitle='',
  [string]$ExpectedTime='', [string]$ActualArrivalTime='', [string]$Verdict='', [string]$Notes='',
  [string]$Serial='emulator-5554', [string]$PackageName='com.ashita_motsumono', [string]$ApkPath='',
...
if($Serial -ne 'emulator-5554'){throw 'serial must be emulator-5554'}
...
switch($Action){
  'Preflight'{$p=PreflightState -RequireApp:$RequireInstalledApp;Json $p (Join-Path $CaseDirectory 'preflight-result.json');if($p.State -eq 'device'){$d=Snapshot 'preflight';Json $p (Join-Path $d 'preflight-result.json')};Write-Host $p.Result}
